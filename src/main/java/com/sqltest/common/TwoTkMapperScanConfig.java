...
@Configuration
@AutoConfigureAfter(TwoTkConfig.class)
public class TwoTkMapperScanConfig {

    @Bean(name = "TwoTkMapperScannerConfigurer")
    public MapperScannerConfigurer TwoTkMapperScannerConfigurer() {
        MapperScannerConfigurer mapperScannerConfigurer = new MapperScannerConfigurer();

        //注意这里的sqlSessionFactory就是Config里面的sqlSessionFactoryBean方法，注解bean的名字
        mapperScannerConfigurer.setSqlSessionFactoryBeanName("TwoTkSqlSessionFactory");
        //接口路径，通过这些接口调用sql的配置，操作数据库
        mapperScannerConfigurer.setBasePackage(ProjectConstant.MAPPER_PACKAGE_NHTEST);
